...
class Preprocessor:
    # Path constants
    PLUG_DIR = Path('preprocessing_python')
    RESOURCE_DIR = PLUG_DIR.joinpath('resource')
    # 2 above line create for app understand path of Preprocessor
        ...
        if sentence != '':
            ...
            if self.__url_regex is None:
                self.__url_regex = re.compile(
                    self.__str_lst[Preprocessor.URL_REGEX_STR])
            url_ranges = [
            ...
            for (url_st_idx, url_ed_idx) in url_ranges:
                # Get plain text
                tmp = sentence[st_idx:url_st_idx]
                if len(tmp) > 0:
                    # There is plain text before current URL
                    # Preprocess plain text
                    tmp = Preprocessor.__process_plaintext(self, tmp)
                else:
                    # No plain text before current URL
                    # Change st_idx to check the next string after current URL
                    st_idx = url_ed_idx + 1

                # Append URL
                tmp += Preprocessor.__process_url(
                    sentence[url_st_idx:url_ed_idx+1])
                # Append plain text / URL to result string
                str_result += tmp
            ...
            if out_file.exists() and not options['overwrite']:
                ...
                with open(out_file, 'w', encoding='utf-8') as fo:
                    for idx in range(0, sents_cnt):
                        fo.write(sents[idx])
                        if idx < sents_cnt - 1:
                            fo.write(Preprocessor.DELIM_RF)
        else:
    ...
    def normalize_accent(sent):
        ...
        """
        new_sent = vn.normalize_u(sent, sign_type=1).strip()
    ...
    def segment_to_sentences(self, text, preprocess=False, replace_y_i=False):
        ...
        # Get honorifics list
        honorifics = Preprocessor.__get_honorifics(
        ...
        for m in punc_matches:
            ...
            tmp = text[st_idx:punc_idx+1].strip()
        ...
        if st_idx < len(text):
            sentence_lst.append(text[st_idx:].strip())
